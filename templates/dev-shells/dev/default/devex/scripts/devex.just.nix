...
in
  pkgs.writeText "devex.just" ''
    ...
    [no-cd]
    [group: "dev shells"]
    @build-dev-shell shell-id="default":
      ${nix} build ".#devShells.$(${nix} eval --raw nixpkgs#stdenv.hostPlatform.system).{{ shell-id }}" && echo "built {{ shell-id }}"
